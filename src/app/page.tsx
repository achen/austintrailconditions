...
export default async function DashboardPage() {
  const trails = await getTrailsWithConditions();
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
        {trails.length === 0 ? (
          ...
          <ul className="divide-y divide-gray-100 bg-white rounded-lg shadow-sm border border-gray-100">
            {trails.map((trail) => {
              const isRideable = trail.condition_status === 'Observed Dry' || trail.condition_status === 'Predicted Dry' || trail.condition_status === 'Open';
                    ...
                    {isDrying && trail.has_active_rain ? (
                      ...
                      <span className="text-xs text-white/70">
                        dry {new Date(trail.predicted_dry_time) > new Date()
                          ...
                          : 'soon'}
                        {trail.remaining_moisture_in != null && ` · ${Number(trail.remaining_moisture_in).toFixed(2)}″ left`}
                      </span>
                    ) : (
                      <span />
                    )}
